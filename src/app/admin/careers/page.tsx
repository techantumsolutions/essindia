...
export default function AdminCareersPortal() {
  ...
  const handleSaveJob = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!jobForm.title || !jobForm.department || !jobForm.description || !jobForm.experience || !jobForm.location || !jobForm.aboutText) {
  ...
  const addListItem = (field: 'requirements' | 'responsibilities' | 'niceToHave' | 'whatWeOffer', text: string, setter: (val: string) => void) => {
    if (!text.trim()) return;
    setJobForm(prev => ({
      ...prev,
  ...
  const removeListItem = (field: 'requirements' | 'responsibilities' | 'niceToHave' | 'whatWeOffer', index: number) => {
    setJobForm(prev => ({
      ...prev,
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">Careers Portal</h1>
      ...
      <div className="flex border-b border-slate-200">
        <button
          ...
          className={`px-5 py-3 text-sm font-bold border-b-2 transition-colors flex items-center gap-2 cursor-pointer ${
            activeTab === 'jobs'
