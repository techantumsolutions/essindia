...
async function patch() {
  console.log('Patching staffing-hero schema...');
  ...
  const migrateContent = (content: any) => {
    ...
    if (content.primaryCta) {
      content.buttonText = content.primaryCta.label || 'Talk to our IT Professionals';
      content.buttonUrl = content.primaryCta.url || '#contact';
      changed = true;
    }

    if (!content.bgColor) { content.bgColor = '#bac7d5'; changed = true; }
    if (!content.badgeBgColor) { content.badgeBgColor = '#ffffff'; changed = true; }
    if (!content.badgeBorderColor) { content.badgeBorderColor = '#2a2d7c'; changed = true; }
    ...
    if (!content.descriptionColor) { content.descriptionColor = '#ffffff'; changed = true; }
    if (!content.buttonBgColor) { content.buttonBgColor = '#ffffff'; changed = true; }
    if (!content.buttonTextColor) { content.buttonTextColor = '#2a2d7c'; changed = true; }
  ...
  console.log('Done patching staffing-hero!');
  process.exit(0);
}
