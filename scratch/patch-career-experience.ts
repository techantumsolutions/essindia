...
async function patch() {
  console.log('Patching career-experience schema...');
  ...
  const migrateContent = (content: any) => {
    ...
    if (Array.isArray(content.products)) {
      content.products = content.products.map((p: any) => {
        const productMigrated = {
          title: p.title || p.name || '',
          image: p.image || p.icon || ''
        };
        changed = true;
        return productMigrated;
      });
    }
  ...
  console.log('Done patching career-experience!');
  process.exit(0);
}
