...
async function main() {
  console.log('🚀 Starting migration of fmcg-logos sections in database...');

  // 1. Update Template Sections
  const tplSecs = await db.select().from(schema.templateSections).where(eq(schema.templateSections.type, 'fmcg-logos'));
  ...
  const libSecs = await db.select().from(schema.sections).where(eq(schema.sections.type, 'fmcg-logos'));
  ...
  const pageSecs = await db.select().from(schema.pageSections).where(eq(schema.pageSections.type, 'fmcg-logos'));
