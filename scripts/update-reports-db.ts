...
async function main() {
  ...
  const tSections = await sql`SELECT id, content_json FROM template_sections WHERE type = 'europe-reports'`;
  ...
  const pSections = await sql`SELECT id, content FROM page_sections WHERE type = 'europe-reports'`;
  ...
  const sections = await sql`SELECT id, content_json FROM sections WHERE type = 'europe-reports'`;
