...
async function main() {
  console.log('🚀 Starting migration of rpa-overview sections in database...');
  ...
  const libSecs = await db.select().from(schema.sections).where(eq(schema.sections.type, 'rpa-overview'));
