...
async function main() {
  console.log('🚀 Starting migration of retail-clients sections in database...');

  // 1. Update Template Sections
  const tplSecs = await db.select().from(schema.templateSections).where(eq(schema.templateSections.type, 'retail-clients'));
  ...
  const libSecs = await db.select().from(schema.sections).where(eq(schema.sections.type, 'retail-clients'));
  ...
  const pageSecs = await db.select().from(schema.pageSections).where(eq(schema.pageSections.type, 'retail-clients'));
